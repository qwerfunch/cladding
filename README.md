...
>
> **Engine: bundled vs. global.** The **Claude Code** marketplace plugin is self-contained — it ships the engine in the plugin and launches it via `${CLAUDE_PLUGIN_ROOT}/dist/clad.js`, so it needs **no** global install. The **Codex / Gemini / Cursor** channels currently wire their MCP server to the `clad` command on your `PATH`, so those hosts still need `npm install -g cladding` (step **a**). Bundling those lanes is tracked for a later release.
