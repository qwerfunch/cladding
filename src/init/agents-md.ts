...
export function renderAgentsMdManagedBlock(spec: Spec | null, cwd: string = '.'): string {
  ...
  return [
    ...
    '- Run `clad check --strict` to verify spec ↔ code across every drift detector.',
    renderDocPointers(cwd).replace(/\n$/, ''),
    '',
    '## Feature cycle — one at a time',
    '',
    'Finish ONE feature end-to-end before the next: author its shard (`acceptance_criteria`',
    '+ `modules`) → implement → author tests in a separate context → `clad done <featureId>`',
    '(sets `status: done` only when `clad check --tier=pre-push --strict` is GREEN). Do not',
    'author shards ahead of their code, or hand-write `status: done`.',
    '',
