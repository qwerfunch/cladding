...
- **직접 에이전트 루프를 짠다면** — cladding은 루프가 딛고 서는 정직한 멈춤 조건·피드백 신호·작업 기억이다([에이전트 루프 검증자](#에이전트-루프의-검증자로-쓰기) 참고).
...
직접 에이전트 루프를 짜는가 — 하네스든 오케스트레이터든? cladding은 그 루프가 딛고 서는 정직한 **멈춤 조건 · 피드백 신호 · 작업 기억**이다: 루프를 대신 돌리지 않고, 무엇이 아직 잘못됐는지 그리고 언제 *진짜로* 멈춰도 되는지를 — 사람의 커밋을 지키는 바로 그 게이트로 — 알려 준다.
...
- **루프 기억** — 로컬 이벤트 로그(`.cladding/events.log.jsonl`, git 추적 제외)가 반복들 사이에 게이트 실행 · done 시도 · 어긋남 발화를 작업 기억으로 담는다 (영속적 기록은 아니고, 5 MB에서 회전한다).

정직한 경계: 이건 루프의 멈춤 조건과 피드백 신호를 단단하게 할 뿐 모델의 코드 품질을 올리지 않는다 — **거버넌스는 정확성과 직교한다**, cladding 자신의 A/B 기록이 그 영수증이다.

<!-- ─────────────── 프로젝트 그래프 ─────────────── -->

## 프로젝트 그래프 — 이제 눈으로 보고 물어본다 <sub>신규</sub>
...
**Spec — 프로젝트의 장기기억.** LLM은 세션이 바뀌면 아무것도 기억하지 못하므로, 스펙은 cladding이 매 세션 전에 주입하는 *의도*의 지속적·git 버전관리 저장소다 — 코드 뒤의 *why*와 *what*(바로 아래 설계 계층이 *how*를 담는다), 사건의 로그가 아니다. 위에서 아래로 4계층 — 의도(A)는 사람 승인 전엔 봉인, 그다음 설계(B), 코드+attestation(C), 감사(D); **A가 모든 계층보다 우선**이라, 스펙과 코드가 다르면 틀린 쪽은 *코드*다.
...
…아니면 init 때처럼 그냥 말하면 된다:

```
[AI 도구 안] cladding 최신 버전으로 업데이트해줘
```

어느 쪽이든 코드 · `spec.yaml` · 문서는 그대로 둔다 — 더 엄격해진 버전은 **짚어만 준다**, 스스로 막거나 고치지 않는다. 새 어긋남을 짚어 주면, 그것도 같은 방식으로 넘기면 된다:
