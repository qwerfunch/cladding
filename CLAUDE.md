...
As of 0.5.0 (PR-A.3) Phase E additionally emits per-host sub-agent manifests from the canonical persona spec — never hand-edit any of these either:

- `plugins/claude-code/.claude-plugin/plugin.json` `agents[]` array
- `plugins/codex/agents/<id>.toml`
- `plugins/cursor/modes.json`
- `plugins/gemini-cli/agents/<id>.md`

## Persona + routing + capabilities (0.5.0)

The work-transaction wires PR-A.1 (routing), PR-A.2 (host hints + detect), PR-A.3 (dispatch hint), and PR-B (capability envelope + dispatch mode + parallel groups) together. The canonical sources are:

- **Persona prompts** — `src/agents/<id>.md` with extended frontmatter (model / permissionMode / sandbox_mode / maxTurns / skills / isolation).
- **Persona resolver** — `agents/routing.yaml` (rules) + `src/agents/routing.ts` (matcher). Rules use `module_prefix` + `intent_tokens`; first match wins; default falls through to `'specialists'`.
- **Capability translator** — `src/agents/capabilities.ts` + `capability-map.ts`. `translateCapabilities(persona, host)` emits the host-shaped envelope. Add a new host: extend the discriminated union + the `translateCapabilities` switch.
- **Host detection** — `src/agents/host-detect.ts` reads env signals. New host: add to `HostName` union + `TIER_MAP` + detection priority list.

When adding a new persona: write `src/agents/<id>.md`, then `npm run build:plugin` mirrors it to all 4 host manifests automatically.
...
- 0.5.0 architecture (host-agnostic multi-agent first): `docs/0.5.0-architecture.md`
- Gemini → Antigravity migration: `docs/migration/gemini-to-antigravity.md`
