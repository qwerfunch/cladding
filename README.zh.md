...
- **自己在搭智能体循环** —— cladding 就是这套循环所倚靠的诚实的停止条件、反馈信号与工作记忆（见 [智能体循环验证者](#把-cladding-用作你智能体循环的验证者)）。
...
自己在搭一套智能体循环 —— 用你自己的 harness 或编排器？cladding 就是这套循环所倚靠的、诚实的**停止条件、反馈信号与工作记忆**：它不替你跑循环，而是用守护人类提交的同一道门禁，告诉循环还有什么不对、以及什么时候*真正*可以停下来。
...
诚实的边界：这加固的是循环的停止条件与反馈信号，而非模型的代码质量 —— **治理与正确性正交**，cladding 自己的 A/B 记录就是凭据。
...
**Spec —— 项目的长期记忆。** LLM 跨会话什么都不记得，所以 spec 是 cladding 在每次会话前注入的*意图*的持久、受 git 版本管理的存储 —— 代码背后的 *why* 与 *what*（紧邻其下的设计层承载 *how*），而不是一份事件日志。自上而下四层 —— 意图（A）在人签字前封存，然后设计（B）、代码 + attestation（C）、审计（D）；**A 高于一切**，因此 spec 与代码不一致时，错的是*代码*。
...
…或者直接说，就像你当初跑 init 那样：

```
[在你的 AI 工具中] 把 cladding 更新到最新版本
```

无论哪种方式，你的代码 · `spec.yaml` · 文档都原封不动 —— 更严格的版本只是把东西**指出来**，既不拦截，也不擅自修改。若它标出了新的漂移，用同样的方式把它交出去：
