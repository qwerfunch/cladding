...
LLM 会分析领域、创建 spec、文档和策略，然后提出 2–3 个后续问题。
...
> **初始化完成后，直接在同一段对话中继续开发即可。** 用自然语言提出下一个功能，AI 会依据生成的 spec 和文档开发，cladding 则在后台运行验证循环。
...
更新会保留代码、`spec.yaml` 和文档。如果新版本报告了漂移，把结果交给 AI 工具即可：
