...
- `--json` — emit the raw `DoctorReport` shape instead of the formatted text surface; the additive shape (`{cwd, events, sentinelMiss, governance, hooks}`) is the stable wire format for MCP clients and follow-up tooling.
