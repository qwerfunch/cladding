...
4. Show `plannedChanges` and `confirmationQuestion` to the user, then stop and wait for a separate reply. The original initialization request is not confirmation.
5. Only after an affirmative user reply, call `clad_init` with the one-time token, the draft, and that reply verbatim as `confirmation`.
6. Ask the returned `nextQuestion` verbatim. Continue with the Cladding clarify workflow after the user answers.
