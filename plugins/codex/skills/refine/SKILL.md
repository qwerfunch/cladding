...
3. User replies
4. Orchestrator runs `clad refine <user's reply>` (positional, no quotes needed)
5. `clad refine` marks the question answered, re-runs the LLM with the full Q-A history, refines artifacts, may add new follow-up questions
6. Loop until `status: done`

## Variadic positional

```
clad refine 법인 사업자만 (개인사업자 제외)
clad refine "한국 시장 위주"
clad refine 카드 + 간편결제 우선
```
...
- Pulse summary line — `✓ refine answered · mode: greenfield · source: llm`
...
- Closing line — `남은 질문: N 개. clad refine <답변> 으로 계속.` OR `✓ 모든 질문에 답변 완료 — 온보딩 종료.`
