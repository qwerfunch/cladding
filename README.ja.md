...
- **自分でエージェントループを組むなら** — cladding はそのループが乗る、正直な停止条件・フィードバック信号・作業記憶だ（[エージェントループの検証役](#エージェントループの検証役として使う)を参照）。
...
自分でエージェントループを組んでいる — 自前のハーネスやオーケストレーターで? cladding はそのループが乗る、正直な **停止条件・フィードバック信号・作業記憶** だ: ループを代わりに回すのではなく、まだ何が間違っていて、いつ *本当に* 止まってよいかを — 人間のコミットを守るのと同じゲートで — 伝える。
...
正直な線引き: これが固くするのはループの停止条件とフィードバック信号であって、モデルのコード品質ではない — **ガバナンスは正確性と直交する**。cladding 自身の A/B 記録がその領収書だ。
...
**Spec — プロジェクトの長期記憶。** LLM はセッションをまたいで何も覚えていないので、spec は cladding が毎セッション前に注入する *意図* の永続的・git 管理された保管庫だ — コードの背後の *why* と *what*（すぐ下の設計層が *how* を持つ）、出来事のログではない。上から下へ 4 階層 — 意図 (A) は人の承認まで封印、次に 設計 (B)、コード + attestation (C)、監査 (D);**A がすべてに優先**するので、spec とコードが食い違えば間違っているのは *コード* の方だ。
...
…あるいは、init のときと同じように頼むだけでいい:

```
[AI ツールの中で] cladding を最新版に更新して
```

どちらの道でも、あなたのコード · `spec.yaml` · ドキュメントには手を触れない — より厳しいバージョンは、指摘すべきことがあっても **指摘するだけ** で、自分でブロックも修正もしない。新たな乖離が見つかったら、それも同じやり方で引き継げばいい:
