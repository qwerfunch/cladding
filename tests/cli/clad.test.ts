...
describe('cli/clad — handler exports', () => {
  ...
  beforeEach(() => {
    ...
    // runCheckCommand sets process.exitCode (not process.exit) so large stdout
    // flushes before exit — reset it per test, and restore after so a recorded
    // failure code can't leak into vitest's own exit status.
...
describe('cli/clad — createProgram', () => {
  ...
  test('program version matches current package version', () => {
    const program = clad.createProgram();
    expect(program.version()).toBe('0.6.3');
  });
});
