...
describe('core/checkpoint', () => {
  ...
  describe('recordCheckpoint', () => {
    test('writes one feature_checkpoint event with the captured fields', () => {
      writeFileSync(join(dir, 'spec.yaml'), 'schema: "0.1"\nfeatures: []\n');
    ...
    test('two checkpoints for the same feature both persist (latest wins on lookup)', () => {
      writeFileSync(join(dir, 'spec.yaml'), 'schema: "0.1"\nfeatures: []\n');
  ...
  describe('recordRollback', () => {
    test('writes one feature_rolled_back event referencing the checkpoint', () => {
      writeFileSync(join(dir, 'spec.yaml'), 'schema: "0.1"\nfeatures: []\n');
      const cp = recordCheckpoint(dir, 'F-001');
