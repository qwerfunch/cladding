...
// did not read src/core/run-sync.ts, src/cli/scan/dispatcher.ts or
// scripts/check-node-surface.mjs). What the ACs pin:
//
...
// run, so they are asserted against the manifest and the built artifact; the
...
const bundlePath = resolve(repoRoot, 'dist', 'clad.js');
const surfaceScript = resolve(repoRoot, 'scripts', 'check-node-surface.mjs');
const hasBundle = existsSync(bundlePath);

// PATH probe for the portable-resolution case, deliberately NOT going through
// the subject under test: `which`/`where` decides whether npm exists at all, so
// a genuine absence skips while a resolution failure inside the runner fails.
const npmOnPath = (() => {
...
describe('core/run-sync — the child-process runner contract', () => {
  ...
  // rather than the raw platform spawn. An ENOENT here is the regression, so it
  // fails loudly instead of reading as an environment skip — the skip is gated
  // on an independent PATH probe, not on the runner's own verdict.
  test.skipIf(!npmOnPath)(
    ...
    () => {
      ...
      if (result.code === 'ENOENT') {
        expect.fail(
          'the runner could not resolve npm (ENOENT) although npm is on PATH — ' +
            'portable command resolution regressed to the raw platform spawn',
        );
      }
...
describe('the declared Node floor', () => {
  ...
  test.skipIf(!hasBundle)(
    '[covers:F-203a3114/AC-ff87f215] the surface check passes against the built bundle on this release',
    () => {
  ...
  test.skipIf(!hasBundle)(
    "[covers:F-203a3114/AC-ff87f215] the bundle imports none of the removed dependency's newest-release surfaces",
    () => {
...
describe('a missing platform capability stays scoped to its feature', () => {
  // Message-contract guard, plain `test(...)` on purpose: the coverage harvester
  // only reads titles off a bare test call, so a skipIf chain here would claim
  // nothing. A test run cannot downgrade the running release, so this asserts
  // the scoped wording — the capability plus the release that provides it, plus
  // the reassurance that the rest of the tool keeps working. It reads the built
  // bundle when there is one and the dispatcher source otherwise, so a fresh
  // checkout still gets a real assertion instead of a vacuous pass. The
  // behavioral proof is the container matrix recorded in the spec.
  test('[covers:F-203a3114/AC-dfc9fd7b] the bundle names the missing capability and leaves every other command working', () => {
    const subject = hasBundle
      ? readBundle()
      : readFileSync(resolve(repoRoot, 'src', 'cli', 'scan', 'dispatcher.ts'), 'utf8');
    expect(subject).toContain('needs built-in network fetch');
    expect(subject).toContain('Every other command works on this release');
  });
});
