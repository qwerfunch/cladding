...
## acme-payments — what this project is

PCI-safe payment orchestration for small merchants.
...
- Tests run under **jest** — keep new tests where this project already keeps them.
- New feature work targets the **trunk** branch by default.
- Never introduce these identifiers: `child_process`, `eval(`.
- Preferred patterns:
  - handling money amounts → prefer integer minor units (cents) over floating-point dollars.
  - a new HTTP handler → prefer zod-validated request bodies.

## Personas — cross-host capability map (anti-self-cert)

The agent that writes a unit of work must not sign off on it. The default persona for this project is **developer**. Each
