...
5. Only when the user's separate reply exactly matches `approvalChallenge`, call `clad_init` with the one-time token, the draft, and that reply verbatim as `confirmation`. Questions, paraphrases, and generic acknowledgements are not approval.
